...
<html
  ...
  <body>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 text-capitalize">Fournisseur data</h5>
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalAjoutFournisseur">
                        Ajouter Fournisseur
                    </button>
                </div>
                <div class="d2c_dataTable table-responsive text-center">
                    <table id="fournisseurs-table" class="display w-100">
                        <thead>
                            <tr>
                                <th>#</th>
                                ...
                                <th>Adresse</th>
                                <th>Email</th>
                                <th>N° Produit Fournis</th>
                                <th>Action</th> <!-- Removed inline style -->
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $fournisseur->téléphone }}</td> <!-- Corrected margin style -->
                                <td>{{ $fournisseur->adresse }}</td>
                                <td>{{ $fournisseur->email }}</td> <!-- Corrected margin style -->
                                @if($fournisseur->nbreProduitFournis === 0)
                                <td class="badge bg-label-danger">{{ $fournisseur->nbreProduitFournis }}</td>
                                @else
                                <td class="badge bg-label-success">{{ $fournisseur->nbreProduitFournis }}</td>
                                @endif
                                <td class="action-buttons">
                                    <div id="actions{{ $fournisseur->id }}">
                                        <a href="{{ route('fournisseurs.edit', $fournisseur->id) }}" class="btn btn-edit btn-sm me-2">
                                            <i class="far fa-edit"></i>
                                        </a>
                                        <form action="{{ route('fournisseurs.destroy', $fournisseur->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-delete btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce fournisseur?');">
                                                <i class="far fa-times-circle"></i>
                                            </button>
                                        </form>
                                        <button class="btn btn-edit btn-sm me-2" onclick="afficherMessage('{{ $fournisseur->id }}')"><i class="far fa-envelope"></i> Envoyer</button>
                                    </div>
                                    <!-- Form to send message -->
                                    <div id="form{{ $fournisseur->id }}" style="display: none;">
                                        <form method="POST" action="{{ route('fournisseurs.mail', $fournisseur->email) }}">
                                            @csrf
                                            <!-- Champ de saisie du message -->
                                            <input type="text" name="message" placeholder="Description du message" required class="form-control mt-2">
                                            <!-- Bouton de soumission du formulaire -->
                                            <button type="submit" class="btn btn-primary mt-2"><i class="far fa-paper-plane"></i> Envoyer</button>
                                            <button type="button" class="btn btn-secondary mt-2" onclick="cacherMessage('{{ $fournisseur->id }}')">Annuler</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
    ...
    <!-- Fin de la table des fournisseurs -->
</div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalAjoutFournisseurLabel">Ajouter Fournisseur</h5>
                <div class="logo-container" style="padding-left:10px;">
                    <img src="./images/fournisseur.png" alt="Logo" class="modal-logo">
                </div>
            ...
            <div class="modal-body">
                <form action="{{ route('fournisseurs.store') }}" method="post">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="adresse" name="adresse" placeholder="Entrer l'adresse" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label" for="telephone">Téléphone</label>
                        <div class="col-sm-10">
                            <input type="tel" class="form-control" id="telephone" name="telephone" placeholder="Entrer le numéro de téléphone" required>
                        </div>
                    </div>
                    <div class="row mb-3">
...
function afficherMessage(fournisseurId) {
    // Cacher les actions
    $('#actions' + fournisseurId).hide();
    
    // Afficher le formulaire spécifique
    $('#form' + fournisseurId).show();
}

function cacherMessage(fournisseurId) {
    // Cacher le formulaire
    $('#form' + fournisseurId).hide();

    // Afficher les actions
    $('#actions' + fournisseurId).show();
}
</script>

...
<html
  ...
  <head>
    ...
    <link rel="stylesheet" href="./assets/css/responsive.css">
        ...
        border-color: #7f8c8d; /* Gris bleu */
    }

    .btn-view:hover {
        color: #fff; /* Couleur de texte blanc au survol */
        background-color: #001f3f; /* Bleu marine au survol */
        border-color: #001f3f; /* Bleu marine au survol */
    }
        .btn-edit {
            color: blue;
            border: 1px solid #ccc;
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 text-capitalize">Client Data</h5>
                    <div>
                        <!-- Modal Trigger Button -->
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ajouterClientModal">Ajouter Client</button>
                    </div>
                </div>
                <div class="d2c_dataTable table-responsive">
                    <table id="clients-table" class="table table-borderless text-center w-100">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Nom Complet</th>
                        ...
                        <tr>
                            ...
                            <td class="badge bg-label-primary" style="margin-top:15px;">{{ $client->email }}</td>
                            <td class="action-buttons">
                                <div id="actions{{ $client->id }}">
                                    <a href="{{ route('clients.edit', $client->id) }}" class="btn btn-edit btn-sm me-2">
                                    ...
                                    <button class="btn btn-edit btn-sm me-2" onclick="afficherMessage('{{ $client->id }}')"><i class="far fa-envelope"></i> Envoyer</button>
                                </div>
                                <!-- Form to send message -->
                                <div id="form{{ $client->id }}" style="display: none;">
                                    <form method="POST" action="{{ route('clients.mail', $client->email) }}">
                                        @csrf
                                        ...
                                        <button type="button" class="btn btn-secondary mt-2" onclick="cacherMessage('{{ $client->id }}')">Annuler</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ajouterClientModalLabel">Ajouter Client</h5>
                <div class="logo-container" style="padding-left:10px;">
                    <img src="./images/client.png" alt="Logo" class="modal-logo">
                </div>
            ...
            <div class="modal-body">
                <form action="{{route('clients.store')}}" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="telephone" name="telephone" placeholder="Entrer numéro de téléphone" />
                    </div>
                    <div class="mb-3">
...
<script>
function afficherMessage(clientId) {
    // Cacher les actions
    $('#actions' + clientId).hide();
    
    // Afficher le formulaire spécifique
    $('#form' + clientId).show();
}

function cacherMessage(clientId) {
    // Cacher le formulaire
    $('#form' + clientId).hide();

    // Afficher les actions
    $('#actions' + clientId).show();
}
</script>
</body>
</html>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-capitalize">Achats data</h5>
                <div>
                    <!-- Modal Trigger Button -->
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ajouterAchatModal">Ajouter Achat</button>
                </div>
            </div>
            <div class="d2c_dataTable table-responsive text-center"> <!-- Ajouter la classe text-center ici -->
                <table id="achats-table" class="display w-100">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Fournisseur</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if($achat->quantite > 0)
                            <td class="badge bg-label-success" style="margin-top:15px;">{{$achat->quantite}}</td>
                            @else
                            <td class="badge bg-label-danger" style="margin-top:15px;">{{$achat->quantite}}</td>
                            @endif
                            <td>{{$achat->quantiteAvantAjouter}}</td>
                            <td class="badge bg-label-primary" style="margin-top:15px;">{{$achat->prix}}</td>
                            <td>{{$achat->date_achat}}</td>
                            <td class="action-buttons">
                                <div class="d-flex align-items-center">
                                    <a href="{{ route('achats.edit', $achat->id) }}" class="btn btn-edit btn-sm me-2">
                                        <i class="far fa-edit"></i>
                                    </a>
                                    <form action="{{ route('achats.destroy', $achat->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-delete btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet achat?');">
...
<!-- Fin de la table des achats -->

<!-- Modal Structure -->
<div class="modal fade" id="ajouterAchatModal" tabindex="-1" aria-labelledby="ajouterAchatModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ajouterAchatModalLabel">Ajouter Achat</h5>
                <div class="logo-container" style="padding-left:10px;">
                    <img src="./images/achat.png" alt="Logo" class="modal-logo">
                </div>
            ...
            <div class="modal-body">
                <form action="{{route('achats.store')}}" method="post">
                    @csrf
                    <div class="mb-3">
                        <label for="clientSelect" class="form-label">Fournisseur</label>
                        <select id="clientSelect" name="nomFournisseur" class="form-select">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nomProduit" name="nomProduit" placeholder="Entrer le nom de produit" />
                    </div>
                    <div class="mb-3">
                        <label for="quantite" class="form-label">Quantité</label>
                        <input type="number" class="form-control" id="quantite" name="quantite" placeholder="0" />
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="dateAchat" name="dateAchat" placeholder="Entrer la date d'achat" />
                    </div>
                    <div class="d-grid">
